...
func (api *ServiceAccountsAPI) CreateToken(c *models.ReqContext) response.Response {
	...
	if api.cfg.ServiceAccountExpirationDayLimit > 0 {
		dayExpireLimit := time.Now().Add(time.Duration(api.cfg.ServiceAccountExpirationDayLimit) * time.Hour * 24).Truncate(24 * time.Hour)
