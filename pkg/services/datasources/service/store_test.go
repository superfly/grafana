...
func TestIntegrationDataAccess(t *testing.T) {
	...
	t.Run("DeleteDataSourceAccessControlPermissions", func(t *testing.T) {
		...
		errAddPermissions := db.WithTransactionalDbSession(context.Background(), func(tx commonSession.Tx[*sqlstore.DBSessionTx]) error {
		...
		permCount := int64(0)
		errGetPermissions := db.WithTransactionalDbSession(context.Background(), func(tx commonSession.Tx[*sqlstore.DBSessionTx]) error {
			sess := tx.ConcreteType()
			var err error
