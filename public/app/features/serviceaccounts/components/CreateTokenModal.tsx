...
export const CreateTokenModal = ({ isOpen, token, serviceAccountLogin, onCreateToken, onClose }: Props) => {
  ...
  if (config.serviceAccountExpirationDayLimit !== undefined) {
    maxExpirationDate.setDate(maxExpirationDate.getDate() + config.serviceAccountExpirationDayLimit + 11);
  }
  const defaultExpirationDate =
    config.serviceAccountExpirationDayLimit !== undefined && config.serviceAccountExpirationDayLimit > 0;
